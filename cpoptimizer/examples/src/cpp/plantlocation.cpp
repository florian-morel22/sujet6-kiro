...
an idea of the performance of the model.  Here, there are two indicators of interest,
both of which indicate in different ways how efficiently the
...
of the plant where this ratio is the smallest.  The KPIs are displayed in the log
whenever an improved solution is found and at the end of the search.
...
int main(int argc, const char * argv[]) {
  ...
  try{
    ...
    IloCP cp(model);
    cp.addKPI(IloSum(demand) / IloScalProd(open, capacity), "Mean occupancy");
    IloNumExprArray usage(env, nbLocation);
    for(l = 0; l < nbLocation; l++)
      usage[l] = load[l] / capacity[l] + (1 - open[l]);
    cp.addKPI(IloMin(usage), "Min occupancy");
