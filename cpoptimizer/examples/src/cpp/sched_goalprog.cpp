...
This two-step approach focuses the first phase on the most important
...
void CreateModel(const char* dataFile,
  ...
  if(!input) {
    env.out() << "usage: sched_goalprog <file> <useGoalProgramming> <failLimit>" << std::endl;
    throw FileError();
  }
...
IloNum SolveGoalProgramming(const char* dataFile, IloInt failLimit) {
  IloEnv env;
  IloModel model(env);
  IloIntArray2 costs;
  ...
  cp.setParameter(IloCP::FailLimit, failLimit/2);
  cp.setParameter(IloCP::NoOverlapInferenceLevel, IloCP::Extended);
  cp.solve();
  IloNum cost1 = cp.getObjValue();
  IloInt failsLeft = failLimit - cp.getInfo(IloCP::NumberOfFails);
  ...
  model.add(obj2);
  cp.setParameter(IloCP::FailLimit, failsLeft);
...
IloNum SolveBasic(const char* dataFile, IloInt failLimit) {
  IloEnv env;
  IloModel model(env);
  IloIntArray2 costs;
  IloIntervalVarArray2 machines;
...
int main(int argc, const char* argv[]) {
  ...
  try {
    ...
    if (useGoalProgramming) {
      ...
      cost = SolveGoalProgramming(filename, failLimit);
    } else {
      std::cout << "Solving in a single step  ..." << std::endl;
      cost = SolveBasic(filename, failLimit);
    }
