// -------------------------------------------------------------- -*- C++ -*-
// File: ./examples/src/cpp/facility_explanations.cpp
// --------------------------------------------------------------------------
...
int main(int argc, const char* argv[]){
  ...
  try{
    ...
    for (i = 0; i < nbStores; i++) {
      sprintf(name, "S_%ld", (long)i);
      supplier[i].setName(name);
    }
    IloIntVarArray open(env, nbLocations, 0, 1);
    for (i = 0; i < nbLocations; i++) {
      sprintf(name, "O_%ld", (long)i);
      open[i].setName(name);
    }
    for (i = 0; i < nbStores; i++)
      ILOADD(model, open[supplier[i]] == 1);
    for (j = 0; j < nbLocations; j++)
      ILOADD(model, IloCount(supplier, j)  <= capacity[j]);
